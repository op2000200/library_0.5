...
int userID2;
QString Curpath3 = "C:\\DB\\shared\\books"+ QDate::currentDate().toString().replace(QRegularExpression("[ ]"), "_")+"_"+QTime::currentTime().toString().replace(QRegularExpression("[:]"), "_")+".txt";
QFile file5("C:\\DB\\shared\\books.txt");
...
{
    userID2 = ID;
    QString inf = "C:\\DB\\user\\" + QString::number(userID2) + "\\info.txt";
    QFile info(inf);
...
void reader::refresh(QFile &f)
{
    ...
    QFile list("C:\\DB\\user\\" + QString::number(userID2) + "\\bookfornow.txt");
...
void reader::refresh1(QFile &f)
{
    ...
    QFile list("C:\\DB\\user\\" + QString::number(userID2) + "\\history.txt");
    list.open(QIODevice::ReadOnly | QIODevice::Text);
    list.seek(0);
    q = list.readLine();
    c = q.toInt();
