...
QString Curpath = "C:\\DB\\shared\\books"+ QDate::currentDate().toString().replace(QRegularExpression("[ ]"), "_")+"_"+QTime::currentTime().toString().replace(QRegularExpression("[:]"), "_")+".txt";
    ...
    ui(new Ui::DirectorMainWindow)
{
    ...
    ui->DATA->setText(QDate::currentDate().toString("dd.MM.yyyy"));
...
void DirectorMainWindow::readData(int ID)
{
    ...
    userID1 = ID;
    QString inf = "C:\\DB\\user\\" + QString::number(userID1) + "\\info.txt";
    QFile info(inf);
